...
Show available commands:
    ...
    $ poetry run ckman --help

Show information about inserted YubiKey:

    $ poetry run ckman info

Run ckman in DEBUG mode:

    $ poetry run ckman --log-level DEBUG info
...
To run integration tests, indicate the serial number (given by `ckman list`) of
